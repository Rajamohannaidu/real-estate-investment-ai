...
with st.sidebar:
    ...
    st.markdown(f"""
    <div class="info-card">
        <p style="margin:0; font-size:0.9rem;"><strong>Dataset:</strong> 545 properties</p>
        <p style="margin:0; font-size:0.9rem;"><strong>Models:</strong> 7 algorithms</p>
        <p style="margin:0; font-size:0.9rem;"><strong>Accuracy:</strong> ~80% R²</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <p>AI-powered platform for intelligent real estate investment decisions.</p>
...
if page == "🏡 Home":
    ...
    with col2:
        if st.button("🚀 Start Predicting Now", use_container_width=True):
            st.switch_page("pages/prediction.py")  # Would navigate if using pages
    ...
    with tab2:
        ...
        colors = ['#667eea' if score >= 0.80 else '#764ba2' if score >= 0.75 else '#a0aec0' 
        ...
        fig1.add_trace(go.Bar(
            ...
            y=df_models['R² Score'],
            marker=dict(color=colors),
            text=[f'{score:.2%}' for score in df_models['R² Score']],
            textposition='outside'
        ))
        ...
        styled_df = df_models.style.background_gradient(
            subset=['R² Score'], 
            cmap='Purples'
        ).format({
            'R² Score': '{:.4f}',
    ...
    if st.session_state.chatbot is None:
        ...
        st.markdown("""
        <div class="success-box">
            <p style="margin:0;">✅ AI Advisor is ready! Ask me anything about real estate investing.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        with chat_container:
            for message in st.session_state.chat_history:
                if message['role'] == 'user':
                    ...
                    st.markdown(f"""
                    <div class="feature-card" style="margin-right: 20%;">
                        <p style="margin:0;"><strong>🤖 AI Advisor:</strong></p>
                        <p style="margin:0.5rem 0 0 0;">{message['content']}</p>
                    </div>
                    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        avg_roi_by_type = dashboard_data.groupby('Type')['ROI'].mean().reset_index()
        
        fig2 = go.Figure()
        
        fig2.add_trace(go.Bar(
            x=avg_roi_by_type['Type'],
            y=avg_roi_by_type['ROI'],
            marker=dict(
                color=avg_roi_by_type['ROI'],
                colorscale='Purples',
                showscale=False
            ),
            text=[f'{roi:.1f}%' for roi in avg_roi_by_type['ROI']],
            textposition='outside'
        ))
